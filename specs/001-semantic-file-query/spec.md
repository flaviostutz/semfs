...
- When the filter excludes some files or a file type cannot be chunked meaningfully, those files must be skipped without corrupting the rest of the index.
...
- **FR-004**: The indexing configuration MUST allow callers to scope which files are considered during indexing through a filter pattern.
...
- **FR-014**: Passage query results MUST include the source file path relative to the indexed directory and inclusive line range for each finding.
...
- **FR-021**: The command-line interface MUST look for a `.semfsrc` configuration file in the current working directory by default.
...
- **FR-024**: Failed indexing or query operations MUST return actionable errors when required query text, configuration, model availability, or index state is invalid for the requested action. At minimum, an actionable error MUST identify the failed action, the relevant path, index, or file when known, and the next corrective step the caller can take.
...
- **FR-026**: The example scenarios MUST measure and record elapsed time for indexing and querying on both synthetic datasets, and MUST persist those benchmark artifacts under a single repository-level `benchmarks/` directory so results can later be added to project documentation.
...
- **Chunk Finding**: A passage-level result containing the source file path relative to the indexed directory, start line, end line, and optional excerpt text for a contiguous relevant range.
...
- **SC-007**: The example benchmark suite records indexing time and query time for both the small and large synthetic datasets on every planned benchmark run and writes those artifacts under the repository-level `benchmarks/` directory.
